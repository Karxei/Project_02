...
def reconcile():
    # GET single query or service metadata
    if request.method == "GET":
        query = request.args.get("query")
        # If no query parameter, return reconciliation service metadata (OpenRefine expects this)
        if not query:
            meta = {
                "name": "Places Reconciliation Service",
                "identifierSpace": "http://example.org/id",
                "schemaSpace": "http://example.org/schema",
                "view": {"url": "http://example.org/view/{{id}}"},
                "defaultTypes": [{"id": "Place", "name": "Place"}],
            }
            return jsonify(meta)

        # Otherwise handle single query
        query = query.strip()
    ...
    # POST batch mode
    # Temporary debug logging (helpful while testing with OpenRefine)
    logger.info("Reconcile POST headers: %s", dict(request.headers))
    logger.info("Reconcile POST content_type: %s", request.content_type)
    try:
        logger.info("Reconcile POST form keys: %s", list(request.form.keys()))
    except Exception:
        logger.info("Reconcile POST form keys: <unavailable>")
    logger.info("Reconcile POST raw body (first 200 bytes): %s", request.data[:200])

    # Robustly parse incoming payload:
    # 1) JSON body
    # 2) form field 'queries' (URL-encoded or multipart)
    # 3) raw body as JSON fallback
    payload = None
    try:
        payload = request.get_json(silent=True)
    except Exception as e:
        logger.debug("get_json raised: %s", e)

    if payload is None:
        raw_queries = None
        try:
            # request.form works for application/x-www-form-urlencoded and multipart/form-data
            raw_queries = request.form.get("queries") if request.form else None
        except Exception:
            raw_queries = None

        if raw_queries:
            try:
                payload = {"queries": json.loads(raw_queries)}
            except Exception as e:
                logger.warning("Failed to parse form 'queries' JSON: %s", e)
                payload = {}
        else:
            ...
            try:
                ...
                logger.warning("Failed to parse raw body as JSON: %s", e)
                payload = {}

    queries = payload.get("queries", {}) if isinstance(payload, dict) else {}
    out = {}

    # If queries is a JSON string, parse it
    ...
    # Support both {"q0": {"query": "Paris"}} and {"q0": "Paris"} forms
    for qid, qobj in list(queries.items()):
        try:
            ...
            logger.exception("Error processing query %s: %s", qid, e)
            out[qid] = {"result": [], "error": str(e)}
